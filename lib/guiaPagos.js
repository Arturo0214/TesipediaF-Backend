...
// Fuerza https: MercadoPago rechaza back_urls con http cuando auto_return='approved'.
...
const API = httpsify(process.env.PUBLIC_API_URL || process.env.BACKEND_URL || 'https://api.tesipedia.com');
...
// Normaliza un número de WhatsApp a solo dígitos (formato wa_id). '' si no viene.
const cleanWa = (v) => String(v || '').replace(/\D/g, '');

// ── Crear checkout (pago único) ─────────────────────────────────────
export async function crearCheckout({ productId, email, metodo, waId }) {
  ...
  if (metodo === 'mercadopago') {
    ...
    const pref = await mp.preference.create({
      body: {
        ...
        payer: { email },
        metadata: { productId, email, waId: wa },
        external_reference: `${productId}|${email}|${wa}`,
...
export async function crearCheckoutCart({ items, email, metodo, waId }) {
  ...
  if (metodo === 'mercadopago') {
    ...
    const pref = await mp.preference.create({
      body: {
        ...
        payer: { email },
        metadata: { items: ids.join(','), email, waId: wa },
        external_reference: `cart:${ids.join(',')}|${email}|${wa}`,
        statement_descriptor: 'TESIPEDIA',
        notification_url: API ? `${API}/guias/webhook/mp` : undefined,
...
// Extrae { isCart, items, productId, emailPart, waPart } de un external_reference / metadata.
// Formato: `<productId|cart:ids>|<email>|<waId>` (waId opcional).
function parseRef(externalRef, metaItems) {
  const raw = String(externalRef || '|');
  const [refPart, emailPart, waPart] = raw.split('|');
  const isCart = refPart.startsWith('cart:') || Boolean(metaItems);
  const items = isCart
    ? String(metaItems || refPart.slice(5)).split(',').map((s) => s.trim()).filter(Boolean)
    : [];
  return { isCart, items, productId: isCart ? 'cart' : refPart, emailPart, waPart: waPart || '' };
}
...
export async function verificarPago({ proveedor, ref }) {
  if (proveedor === 'mp') {
    ...
    const { isCart, items, productId, emailPart, waPart } = parseRef(pago.external_reference, pago.metadata?.items);
    return {
      ...
      // MP baja a snake_case las llaves de metadata (waId → wa_id).
      waId: cleanWa(pago.metadata?.wa_id || pago.metadata?.waId || waPart) || null,
  ...
  if (proveedor === 'stripe') {
    ...
    return {
      ...
      waId: cleanWa(session.metadata?.waId) || null,
